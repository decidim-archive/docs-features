...
// OPCIONAL. Data de publicació de la versió. Per defecte s'insereix automàticament en
// aquest camp la data de generació del document web o PDF, en format YYYY-MM-DD. Si es
// desitja, es pot substituir "{docdate}" per una data manual en el format que es vulgui,
// però a partir d'aquest moment s'haurà d'actualitzar també manualment.
:revdate: {docdate}

// OPCIONAL. Veure la definició d'aquest camp a l'apartat "Criteris d'autoria ..." de més
// avall.
:_editors: Xabier E. Barandiaran, Carolina Romero

// OBLIGATORI. Veure la definició d'aquest camp a l'apartat "Criteris d'autoria ..." de
// més avall.
